// scripts/runEnrich.ts — standalone enrichment, bez aliasów @/
...
async function fetchAllRawProcessed(): Promise<any[]> {
  const PAGE = 500;
  const all: any[] = [];
  let offset = 0;
  while (true) {
...
async function fetchAllEnrichedIds(): Promise<Set<string>> {
  const PAGE = 500;
  ...
  while (true) {
    ...
    if (error) throw error;
    if (!data || data.length === 0) break;
    data.forEach((i: any) => ids.add(i.id));
    if (data.length < PAGE) break;
    offset += PAGE;
  }
  return ids;
...
async function run() {
  console.log("=== Standalone enrichBatch START ===");

  const rawItems = await fetchAllRawProcessed();
  const enrichedIds = await fetchAllEnrichedIds();
  const toProcess = rawItems.filter((i: any) => !enrichedIds.has(i.id));
  ...
  for (const item of toProcess) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (isLast) {
          ...
          // czekaj przed retry: 5s, 15s
          const wait = attempt === 1 ? 5000 : 15000;
          process.stdout.write(`\n[RETRY ${attempt}/3 za ${wait/1000}s] ${item.name_raw}\n`);
    ...
    if (ok) {
      success++;
      process.stdout.write(`\r✓ ${success}/${toProcess.length} | błędy: ${failed}  `);
    }
  }

  console.log(`\n\n=== KONIEC === sukces: ${success}  błędy: ${failed}`);
}
