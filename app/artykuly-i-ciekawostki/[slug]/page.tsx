...
export default function ArticleSlugPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    fetch(`/api/content/${slug}?type=article`)
      .then((r) => {
        if (!r.ok) { setNotFound(true); return null; }
        return r.json();
      })
      .then((d) => d && setArticle(d.item));
  }, [slug]);
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      >
        <div className="px-4 md:px-6 h-14 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-2 text-xs text-white/35">
            <Link href="/artykuly-i-ciekawostki" className="hover:text-white transition-colors">Artykuły i ciekawostki</Link>
            {article && (
              <>
                <span>/</span>
                <span className="text-[#B85C38] truncate max-w-[160px]">{article.title.split("–")[0].trim()}</span>
              </>
            )}
          </nav>
        </div>
      ...
      <main className="pt-20 pb-20 px-4 md:ml-60">
        <div className="max-w-3xl mx-auto">
          ...
          {notFound ? (
            <p className="text-white/50">Nie znaleziono artykułu.</p>
          ) : !article ? (
